...
const MyPagination = (p: {
  ...
  const Pages = (p: { x: number; y: number }) => {
    ...
    const arr = [];
    if (x <= PageCount)
      for (let i = x; i <= Math.min(PageCount, y); i++) {
        arr.push(i);
      }
    ...
    <div className="p-4 flex">
      <Pagination>
        <PaginationContent>
          ...
          {
            // 14 15 16 ... 18
            page >= RightStart - 1 - Math.floor(MaxMid / 2) - 1 ||
            PageCount <= MidEnd + 1 + MaxRight ? (
