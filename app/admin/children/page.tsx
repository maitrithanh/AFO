...
const ParentPage = () => {
  ...
  const PageCount = useMemo(() => {
    return Math.ceil(
      (dataChildren?.filter(searchChild).length ?? 0) / RowPerPage
    );
  ...
  return (
    <>
      ...
      <div className="mb-3 italic">{keyword && `Tìm kiếm "${keyword}": `}</div>
              ...
              .map((x, i) => {
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{x.classRoom}</td>
                    <td className="px-6 py-4">{x.birthDay}</td>
                    <td className="px-6 py-4">{x.gender}</td>
                    <td className="px-6 py-4">{x.parentName}</td>
