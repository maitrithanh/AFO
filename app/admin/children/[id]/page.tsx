...
const DetailChildrenPage = ({ params }: any) => {
    const id = params.id;
    const { data: dataChildren } = useFetch<DetailChildReq>('Child/getChild?id=' + id, null, id);
    ...
    return <div className="w-[60%] mx-auto">
        <div className="flex justify-between ">
            ...
            <label className="inline-flex items-center mb-5 cursor-pointer">
                <input type="checkbox" value="" className="sr-only peer" checked={editChild} onChange={() => { setEditChild(x => !x) }} />
        ...
        <div>
            {child && <ChildrenPutForm data={child} setData={(x) => setChild(x)} editable={editChild} /> }
        </div>
