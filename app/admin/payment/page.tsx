...
const Columns: TableTemplateColumn<PaymentModel>[] = [
    ...
    {
        title: 'ĐẾN NGÀY',
        getData: (x) => <span className={toYMD(x.endTime) < moment().format('yyyy-MM-DD') ? 'text-red-400' : ''}>{x.endTime}</span>
    },
    {
        title: 'PHẢI ĐÓNG',
    ...
    {
        title: 'TRẠNG THÁI',
        getData: (x) => <span className={'text-' + getPaymentStatusStyle(x.statusCode)}>{x.status}</span>
    },
]
...
const PaymentPage = () => {
    ...
    const { data } = useFetch<PaymentModel[]>(`Tuition/GetAllTuition?month=${month}&year=${year}`, refresh);
    ...
    return <TableTemplate<PaymentModel>
        ...
        extraElementsToolBar={<div className="flex">
