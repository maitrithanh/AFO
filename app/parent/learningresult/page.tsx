  ...
  return (
    <div className="h-[600px] bg-white md:w-3/4 m-auto rounded-xl">
      <div className="relative overflow-x-auto  bg-white pt-2 sm:rounded-lg">
        ...
        {dataAttendanceByChild?.map((data: any) => {
          return (
            <div className="p-4" key={data.child.childId}>
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-xl">
                    Họ tên: <span>{data.child.fullName}</span>
                  </p>
                  <p className="text-xl">
                    Đánh giá: <span>{data.child.status}</span>
                  </p>
                </div>
                <table className="border">
                  ...
                  <tbody>
                    <tr className="text-center">
                      <td className="border p-1">{data.child.countCheck}</td>
                      <td className="border p-1">
                        {data.child.countOffReason}
                      </td>
                      <td className="border p-1">{data.child.countNoCheck}</td>
              ...
              <div>
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 max-h-[600px] ">
                  ...
                  <tbody>
                    {data?.checkIn?.map((dataCheckIn: any) => {
                      return (
                        <tr
