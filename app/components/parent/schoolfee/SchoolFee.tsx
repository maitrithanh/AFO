        ...
        <div className="relative py-4 my-4 bg-white flex items-center w-full border-b">

          {
            (data.statusCode == '3' || data.statusCode == '0') ?
              <div className={`mr-2 text-${getPaymentStatusStyle(data.statusCode)}`}>
          ...
          <div>
            <h3 className="text-lg">
              {data.content}: <strong className={`text-${getPaymentStatusStyle(data.statusCode)}`}>{getAmount()}</strong>
              {
                data.paidTime &&
                <span className={`text-${getPaymentStatusStyle(data.statusCode)} italic ml-1`}>
                   (ngày {data.paidTime})
                </span>
              }
            </h3>
              ...
              <AlertDialogContent className="max-h-[90vh] overflow-auto">
                <AlertDialogHeader>
                  <AlertDialogTitle className="flex items-center">
                    ...
                    <p className={`bg-${getPaymentStatusStyle(data.statusCode)} text-sm font-normal ml-4 text-white p-1 rounded-full`}>
                      {data.status}
                    </p>
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    <div>
                      <div className="flex justify-between text-lg text-black">
                        ...
                        <p className="font-bold">{data.total.toLocaleString('en')}</p>
                      </div>
                      <div className="flex justify-between text-lg text-black">
                      ...
                      <div className="flex justify-between text-lg text-black pt-2 border-t">
                        ...
                        <p className="font-bold text-main">{data.total.toLocaleString('en')}</p>
                      </div>

                      <div className="flex justify-between text-lg text-black pt-2 border-t">
                        <p className="mr-2">Đã đóng:</p>
                        <p className="font-bold">{data.paid.toLocaleString('en')}</p>
                      </div>
...

                      {
                        (data.statusCode == '3' || data.statusCode == '0') &&
                        <div>
                            <p className="font-bold text-xl text-center my-3">Thanh toán qua mã QR</p>
