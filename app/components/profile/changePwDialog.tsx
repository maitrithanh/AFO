...
const ChangePwDialog = ({ onClose }: Prop) => { 
    ...
    const onSubmit: SubmitHandler<FieldValues> = (data) => {
        setIsLoading(true);
        callApiWithToken().put('Auth/ChangePassword', data)
            .then((res) => { 
    ...
    return <div onClick={onClickOut} className="fixed top-0 left-0 w-full h-full flex items-center bg-black bg-opacity-40 justify-center">
        <form className="space-y-6 p-10 bg-white rounded-3xl" action="#" method="POST">
            ...
            <div>
                <div className="mt-2">
                    <Input
                        ...
                        register={register}
                        errors={errors}
                        onclick={() => { setHidePasswordOld(x => !x) }}
                        required
                    />
                </div>
            </div>
            ...
            <div>
                <div className="mt-2">
                    <Input
                        ...
                        register={register}
                        errors={errors}
                        onclick={() => { setHidePasswordNew(x => !x) }}
                        required
                    />
                </div>
            </div>
