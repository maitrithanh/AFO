...
const PutSchedulePage = ({ curr, edit }: Props) => {
    ...
    const [selectClass, setSelectClass] = useState<ClassRoom[]>(curr.classes ?? [])
    
    const { data: classes } = useFetch<ClassRoom[]>('classRoom/List/' + new Date().getFullYear().toString());
    ...
    return <div>
        ...
        <div className="w-full mb-5 px-5">
            <h2 className="text-3xl mb-3">{edit ? "Chỉnh sửa " : "Thêm "} Thời khóa biểu</h2>
            ...
            <div className="flex group items-baseline mb-3">
                ...
                <button
                    ...
                    className={`${editName ? "text-green-600" : "invisible"
            ...
            <div className="group pr-2 w-full flex items-center">
                ...
                <button
                    title="Sửa tên menu"
                    className={`${editDesc ? "text-green-600" : "invisible"
                        } ml-2 group-hover:visible`}
        ...
        <div className="p-4 flex items-end">
            <div>
                ...
                {/* chosen classes */}
                <div>
                    {selectClass.map(x => <span className="py-1 px-2 mr-3 mb-1 border-gray-300 text-gray-500 border-solid border-2 rounded-xl">
                        <span className="mr-2">{x.name}</span>
                        <span className="text-black text-xl cursor-pointer" onClick={() => onRemoveClass(x)}>x</span>
                    </span>)}
                </div>
            </div>
